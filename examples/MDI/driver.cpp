...
int me, nprocs;			// all of driver
MPI_Comm world;			// MPI_COMM_WORLD for driver
...
double *cell_displ;			// Cell parameters 
int *elements;			// Send natoms Atomic numbers for every atom 
char *symbols;			// Send symbols for every species
...
double *stress;			// Stress (eV/Ang in LATTE) 
...
engine_wrapper (void *mpicomm_ptr, MDI_Comm mdicomm, void *class_object)
{
  ...
  MDI_Send (fname, 100, MDI_CHAR, mdicomm);
  ...
  // Send cell dislacement (not used by latte. This is just added for compatibility)
  cell_displ = new double[3];
  cell_displ[0] = 0.0;
  cell_displ[1] = 1.0;
  cell_displ[2] = 2.0;
  MDI_Send_Command (">CELL_DISPL", mdicomm);
  MDI_Send (cell_displ, 3, MDI_DOUBLE, mdicomm);
...
main (int narg, char **args)
{
  ...
  int ndriver, psdrivers, nlammps, plammps, nlatte, platte, nfit, pfit;
  ...
  if (ndriver < 1)
    error ("ndriver must be > 0");
  if (psdrivers < 1 || psdrivers > ndriver)
    error ("psdrivers must be > 0 and <= ndriver");
  if (nlammps < 0)
  ...
  if ((ndriver + nlammps * plammps + nlatte * platte + nfit * pfit) !=
      nprocs)
    error ("Nprocs = ndriver + nlammps*plammps + nlatte*platte + "
  ...
  if (me < ndriver)
    color = 0;
  else if (me < ndriver + nlammps * plammps)
    color = (me - ndriver) / plammps + 1;
  else if (me < ndriver + nlammps * plammps + nlatte * platte)
    color = (me - ndriver - nlammps * plammps) / platte + nlammps + 1;
  else if (me <
	   ndriver + nlammps * plammps + nlatte * platte + nfit * pfit)
    color =
      (me - ndriver - nlammps * plammps - nlatte * platte) / pfit +
      nlammps + nlatte + 1;
  ...
  if (color == 0)
      ...
      if (me_intra < psdrivers)
	{
	  ...
	  for (int i = 0; i < nlammps_mine; i++)
		...
		itask = 0;
	      iproc = ndriver + (nlammps_first + i) * plammps;
	  ...
	  for (int i = 0; i < nlatte_mine; i++)
		...
		itask = 0;
	      iproc =
		ndriver + nlammps * plammps + (nlatte_first + i) * platte;
	      MPI_Send (&itask, 1, MPI_INT, iproc, 0, world);
	    }
	  ...
	  for (int i = 0; i < nfit_mine; i++)
		...
		itask = 0;
	      iproc = ndriver + nlammps * plammps + nlatte * platte +
		(nfit_first + i) * pfit;
	      MPI_Send (&itask, 1, MPI_INT, iproc, 0, world);
	    }
	  ...
	  while (complete_lammps < tlammps_mine
		 || complete_latte < tlatte_mine || complete_fit < tfit_mine)
	    {
	      ...
	      if (iproc < ndriver + nlammps * plammps)
		{
		  complete_lammps++;
		  itask = next_lammps++;
		  if (itask > tlammps_mine)
		    ...
		    itask += tlammps_first;
		       ...
		       ndriver + nlammps * plammps + nlatte * platte)
		{
		  complete_latte++;
		  itask = next_latte++;
    ...
    {
      engineflag = LAMMPS;
      int instance = (me - ndriver) / plammps + 1;
    ...
    {
      engineflag = LATTE;
      int instance = (me - ndriver - nlammps * plammps) / platte + 1;
  ...
  else if (color <= nlammps + nlatte + nfit)
    {
      engineflag = FITSNAP;
      int instance =
	(me - ndriver - nlammps * plammps - nlatte * platte) / pfit + 1;
