...
void sp2pure_nospin3(REAL bndfil, int  hdim, REAL *x0_pointer, REAL maxeval,
  ...
  while( breakloop == 0 && iter < 100 ) {
    ...
    // On 1 GPU: xtmp = x0 * x0
    //M_Multiply( &MINUS1, x0, x0, &ZERO, xtmp);

    // On multiple GPUs: xtmp = x0 * x0
    M_MultiplyMgpu( &MINUS1, x0, x0, &ZERO, xtmp, xtmp2);

    // xtmp = xtmp + x0
    M_MultiplyScalarSumMgpu( &ONE, x0, xtmp);
    ...
    // Distribute x0
    M_CollectDistributeMgpu(x0);

    fprintf(stdout, "trace x0 = %f \n", trx);
...
*/
  ...
  M_DeallocateLocal(xtmp2);
  M_DeallocateDevice(xtmp);
  M_DeallocateDevice(xtmp2);
