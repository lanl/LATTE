subroutine V_Kernel_Fermi(D0,dq_dv,v,mu0,mu1,T,RX,RY,RZ,LBox,Hubbard_U,Element_Type,Nr_atoms,MaxIt,eps, &
m,HDIM, Max_Nr_Neigh,Coulomb_acc,TIMERATIO,nnRx,nnRy,nnRz,nrnnlist,nnType,H_INDEX_START,H_INDEX_END, &
H,S,Z,Nocc,Znuc)
...
real(PREC)                     :: D_dq_v(HDIM,HDIM), H_dq_v(HDIM,HDIM), dq_v(Nr_atoms), Fe_vec(HDIM),Fe(HDIM,HDIM)
real(PREC), intent(out)        :: dq_dv(Nr_atoms)
real(PREC)                     :: Q(HDIM,HDIM), ee(HDIM), OccErr, Occ, dOcc, mu_0, beta, Occ_I
...
!    call get_deriv_finite_temp(D_dq_v,H0,H1,Nocc,T,Q,ee,Fe_vec,mu0,eps,HDIM)
