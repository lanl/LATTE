...
!     write(*,*) 'bodirect OCCTARGET = ', OCCTARGET
!     write(*,*) 'bodirect EVALS(OCCTARGET-1:O+1) = ', EVALS(int(OCCTARGET)-1:int(OCCTARGET)+1)
!     write(*,*) 'bodirect EVALS(1:3) = ', EVALS(1:3)
     CHEMPOT = 0.5D0*(EVALS(int(OCCTARGET+.01))+EVALS(int(OCCTARGET+1.01)))
!     write(*,*) 'bodirect CHEMPOT = ', CHEMPOT
!     write(*,*) 'bodirect NUMLIMIT = ', NUMLIMIT
!     write(*,*) 'bodirect BREAKTOL = ', BREAKTOL
...
#else
!     write(*,*) ' BOEVEC OCCERR =', OCCERROR, '   BREAKTOL =', BREAKTOL, ' ITER = ', ITER
     ...
     DO WHILE (ABS(OCCERROR) .GT. BREAKTOL .AND. ITER .LT. 100)
        ...
        OCCERROR = (OCCERROR)/SQRT(1.D0*NATS)
!        write(*,*) ' bodirect OCCERROR = ', OCCERROR
     ...
     DO I = 1, HDIM
       ...
       !! ANDERS SAVE FOR RESPONSE CALCULATIONS
       FERMIOCC(I) = FDIRAC
