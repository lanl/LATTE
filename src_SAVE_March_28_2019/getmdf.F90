...
SUBROUTINE GETMDF(SWITCH, CURRITER)
 ...
 !!! ANDERS CHANGE END
  ...
  ELSE
     IF(VERBOSE >= 1)WRITE(*,*)"Doing QCONSISTENCY ... CURRITER = ", CURRITER
     CALL QCONSISTENCY(SWITCH, CURRITER) ! Self consistent charge transfer
     WRITE(*,*) ' GETMDFTEST_2 QCONSISTENCY ANDERS  CURRITER = ', CURRITER, CURRITER
  ENDIF
...
!  ENDIF
  ...
  IF (ELECTRO .EQ. 1 .AND. QITER .EQ. 0) THEN
     ...
     write(*,*) 'getmdf COULCORRECTION 2q-n =', TWO*DELTAQ(:) - OLDDELTAQS(:)
     write(*,*) 'getmdf OLDDELTAQS =', OLDDELTAQS(:)
     write(*,*) 'getmdf PNK(1,:) =', PNK(1,:)
...
!        ECOUL = ECOUL + (TWO*DELTAQ(I) - OLDDELTAQS(I)) * &    ! Orig
!             (HUBBARDU(ELEMPOINTER(I))*OLDDELTAQS(I) + COULOMBV(I))
        ECOUL = ECOUL + OLDDELTAQS(I)* &    ! Orig
             (HUBBARDU(ELEMPOINTER(I))*OLDDELTAQS(I) + COULOMBV(I))
        !ECOUL  MAYBE NEEDS CHANGE!!! ANDERS
     ENDDO
