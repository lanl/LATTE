...
MODULE MIXER_MOD
  ...
  USE SETUPARRAY
  USE NONOARRAY
  USE DIAGARRAY    ! CHANGE ANDERS
  USE XBOARRAY     ! CHANGE ANDERS
#ifdef PROGRESSON
  USE PRG_PULAYMIXER_MOD
#endif
  PRIVATE

  PUBLIC :: QMIXPRG, KERNELMIXER, KERNELPROPAGATION
...
!    write(*,*) ' Res Mix = ', norm2(Res)/sqrt(1.D0*NATS)
!    if  (PITER <= 25) then  !! typical choice <= 1, for really really hard cases <= 20
  ...
  SUBROUTINE QMIXPRG(PITER)
    ...
    INTEGER :: I,J,NUMORB, INDEX, NRANK
    CHARACTER(20) :: MYMIXERTYPE
    real(LATTEPREC), ALLOCATABLE :: DELTAQBUFF(:)
    ...
    ELSEIF(MYMIXERTYPE == "Kernel")THEN

!        ALLOCATE(DELTAQBUFF(NATS))
!        DELTAQBUFF = DELTAQ
        NRANK = MIN(NORECS,NATS)      
        ! NRANK = 0 -> linear mixing, NRANK = NATS gives exact Newton
        CALL KERNELMIXER(PITER,NRANK)     !ANDERS: Alternative mixing scheme ANDERS
!        DEALLOCATE(DELTAQBUFF) 
