  ...
  DO I = 1, NATS
     ...
     DO NEWJ = 1, TOTNEBPP(I)
        ...
        IF (MAGR2 .LE. RCUT2) THEN
           ...
           KLO = 1
           KHI = PPTABLENGTH(PPSEL)

           DO WHILE (KHI - KLO .GT. 1)

              K = (KHI + KLO)/2

              IF (PPR(K,PPSEL) .GT. MAGR) THEN
                 KHI = K
              ELSE
                 KLO = K
              ENDIF
           ...
           DX = PPR(KHI,PPSEL) - PPR(KLO,PPSEL)

           A = (PPR(KHI, PPSEL) - MAGR)/DX
           B = (MAGR - PPR(KLO, PPSEL))/DX

           TMPE = A*PPVAL(KLO,PPSEL) + B*PPVAL(KHI, PPSEL) + &
                ((A*A*A - A)*PPSPL(KLO,PPSEL) + &
