...
!  TYPE(BML_MATRIX_T) :: ORTHOBO_BML, BO_BML
  ...
  IF (EXISTERROR) RETURN; IF (VERBOSE >= 2) WRITE(*,*)"In deorthomyrhoprogress.F90 ..."
  ...
  IF (SPINON .EQ. 0) THEN  !OPTS FOR COVID
...
!          BO, ORTHOBO_BML, ZERO, LT%MDIM)

     IF(.NOT. BML_ALLOCATED(BO_BML))THEN 
             CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
                  LATTEPREC, HDIM, LT%MDIM, BO_BML)
     ENDIF
     IF(DOKERNEL .OR. DFTBU)THEN 
     !  CALL BML_IMPORT_FROM_DENSE(LT%BML_TYPE, BO, ORTHOBO_BML, ZERO, LT%MDIM)
     ENDIF

     CALL PRG_DEORTHOGONALIZE(ORTHOBO_BML,ZMAT_BML, BO_BML,&
