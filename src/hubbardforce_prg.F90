...
SUBROUTINE HUBBARDFORCEPRG  ! AND ENERGY
  ...
  REAL(LATTEPREC), ALLOCATABLE :: UD(:,:), UP(:,:)
  ...
  ALLOCATE(B(HDIM,HDIM), UD(HDIM,HDIM), UP(HDIM,HDIM))
  ...
  IF (KON==0) then
    ...
    CALL BML_TRANSPOSE_NEW(AUX2_BML, AUX1_BML)
    ...
    ! B + B^T -> Ahub
    CALL BML_TRANSPOSE_NEW(UD_BML, AUX1_BML)
