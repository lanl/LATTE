...
SUBROUTINE ALLOCATEDIAG
  ...
  IF (KON .EQ. 0) THEN
     ...
     IF (SPINON .EQ. 0) THEN
        IF (.NOT. ALLOCATED(EVALS)) ALLOCATE (EVALS(HDIM))
        IF (.NOT. ALLOCATED(DVALS)) ALLOCATE (DVALS(HDIM))
        IF (.NOT. ALLOCATED(EVECS)) ALLOCATE (EVECS(HDIM, HDIM))
        !ALLOCATE (EVALS(HDIM), EVECS(HDIM, HDIM))
        ALLOCATE (FERMIOCC(HDIM))   !! ANDERS_CHANGE
     ELSE
  ...
  ELSE ! Allocate arrays if we're doing k-space
     ...
     IF (SPINON .EQ. 0) THEN
        ALLOCATE (KEVALS(HDIM, NKTOT), KEVECS(HDIM, HDIM, NKTOT), &
             CPLIST(HDIM*NKTOT))
     ELSE
        ALLOCATE(KEVALSUP(HDIM, NKTOT), KEVALSDOWN(HDIM,NKTOT))
        ALLOCATE(KEVECSUP(HDIM, HDIM, NKTOT), KEVECSDOWN(HDIM, HDIM, NKTOT))
     ENDIF
