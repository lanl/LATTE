  ...
  IF (ITER .EQ. 1) THEN
     ...
     IF (ELECTRO .EQ. 1) THEN
       ...
       POrth = DOrth
        ...
        call DGEMM('N','N',HDIM,HDIM,HDIM,TWO,XMAT,HDIM,POrth,HDIM,ZERO,NONOTMP,HDIM)
        call DGEMM('N','T',HDIM,HDIM,HDIM,ONE,NONOTMP,HDIM,XMAT,HDIM,ZERO,D,HDIM)
        do I = 1, NATS
          DELTAQDM(I) = -ATOCC(ELEMPOINTER(I))
          do K = H_INDEX_START(I), H_INDEX_END(I)
              DELTAQDM(I) = DELTAQDM(I) + dot_product(D(:,K),SMAT(:,K))
          enddo
        enddo
  ...
  ELSEIF (ITER .GT. 1) THEN
     ...
     ELSEIF (XBODISON .EQ. 1) THEN
        ...
        ELSEIF (ELECTRO .EQ. 1) THEN
            ...
            POrth = 2.D0*PO_0 - PO_1 + KAPPA_SCALE*KAPPA_XBO*(DOrth-POrth) &
            ...
            call DGEMM('T','N',HDIM,HDIM,HDIM,ONE,XMAT,HDIM,SMAT,HDIM,ZERO,IXMAT,HDIM)
            call DGEMM('N','N',HDIM,HDIM,HDIM,ONE,XMAT,HDIM,POrth,HDIM,ZERO,NONOTMP,HDIM)
            call DGEMM('N','N',HDIM,HDIM,HDIM,ONE,NONOTMP,HDIM,IXMAT,HDIM,ZERO,PS,HDIM)
            call DGEMM('N','T',HDIM,HDIM,HDIM,ONE,NONOTMP,HDIM,XMAT,HDIM,ZERO,D,HDIM)

           do I = 1, NATS
             DELTAQDM(I) = -ATOCC(ELEMPOINTER(I))
             do K = H_INDEX_START(I), H_INDEX_END(I)
                 DELTAQDM(I) = DELTAQDM(I) + TWO*dot_product(D(:,K),SMAT(:,K))
             enddo
           enddo
