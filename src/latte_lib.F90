...
MODULE LATTE_LIB
  ...
  PUBLIC :: LATTE, LATTE_ABIVERSION, COMPUTE
...
!      WRITE(*,*)"STRESS_INOUT",-VIRIAL(1),-VIRIAL(2),-VIRIAL(3) 
  ...
  !!
  SUBROUTINE COMPUTE(COMPFLAG_IN,SY_IDX,NUM_SY,NCORES_IN,NCOREATOMS_IN,ETEMP_IN,CHEMPOT_IN,COULOMBV_IN,COORDS_IN,ATOMTYPES_IN,ATOMIC_NUMBERS_IN,LATTICE_VECTORS_IN,HAM_OUT,OVER_OUT,ZMAT_OUT,EVECTS_OUT,DM_OUT,CHARGES_OUT,EVALS_OUT,DVALS_OUT,ENERGY_OUT,FTOT_OUT,VERB_IN,NEWSYSTEM_IN,KEEPMEM_IN,EXISTERROR_INOUT)
    ...
    REAL(LATTEPREC), ALLOCATABLE, INTENT(INOUT) :: ZMAT_OUT(:,:), EVECTS_OUT(:,:)
    ...
    REAL(LATTEPREC), INTENT(IN) :: CHEMPOT_IN, ETEMP_IN 
...
! #endif
    ...
    KBT = 8.617333262145D-5 * ETEMP_IN 
#ifdef MAKELIBON
    ...
    IF(NEWSYSTEM == 0)THEN
      ...
      IF(.NOT. BML_ALLOCATED(ESTRUCT_LIST(SY_IDX)%HAM0) .OR. .NOT. BML_ALLOCATED(ESTRUCT_LIST(SY_IDX)%OVER))THEN
    ...
    IF (LIBCALLS == 0) THEN
      ...
      IF (KEEPMEM == 1)THEN
        LIBINIT = .TRUE.
        LIBCALLS = LIBCALLS + 1
        CALL BML_EXPORT_TO_DENSE(ESTRUCT_LIST(SY_IDX)%HAM0, H0)
        CALL BML_EXPORT_TO_DENSE(ESTRUCT_LIST(SY_IDX)%HAM, H)
        CALL BML_EXPORT_TO_DENSE(ESTRUCT_LIST(SY_IDX)%OVER, SMAT)
        ...
        CALL BML_IMPORT_FROM_DENSE(BML_MATRIX_DENSE, &
             H, HAM_BML, ZERO, HDIM)
        CALL BML_IMPORT_FROM_DENSE(BML_MATRIX_DENSE, &
             XMAT, ZMAT_BML, ZERO, HDIM)
        IF (COMPFLAG .GT. 1)THEN
          CALL BML_EXPORT_TO_DENSE(ESTRUCT_LIST(SY_IDX)%OHAM, ORTHOH)
          CALL BML_IMPORT_FROM_DENSE(BML_MATRIX_DENSE, &
      ...
      IF (COMPFLAG .EQ. 1)THEN
        CALL BML_IMPORT_FROM_DENSE(BML_MATRIX_DENSE, &
             H0, ESTRUCT_LIST(SY_IDX)%HAM0, ZERO, HDIM)
      ENDIF
      ...
      IF (COMPFLAG .GE. 2)THEN
        CALL BML_IMPORT_FROM_DENSE(BML_MATRIX_DENSE, &
             ORTHOH, ESTRUCT_LIST(SY_IDX)%OHAM, ZERO, HDIM)
        CALL BML_IMPORT_FROM_DENSE(BML_MATRIX_DENSE, &
