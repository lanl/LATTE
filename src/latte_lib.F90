...
CONTAINS
  ...
  FACTORIALLIST(6)=120
  FACTORIALLIST(7)=720
  FACTORIALLIST(8)=5040
  FACTORIALLIST(9)=40320
  FACTORIALLIST(10)=362880

  DO I = 1,10
     REALM1K(I) = REAL((-1)**(I-1),LATTEPREC)
  ENDDO
       ...
       IF(MOD(LIBCALLS,WRTFREQ) == 0)THEN
          IF(VERBOSE >= 1) THEN
             ...
             IF(.NOT. ALLOCATED(SY%COORDINATE))THEN
                ALLOCATE(SY%COORDINATE(3,NATS))
                ALLOCATE(SY%SYMBOL(NATS))
                ALLOCATE(SY%LATTICE_VECTOR(3,3))
                ALLOCATE(SY%NET_CHARGE(NATS))
             ENDIF
             SY%COORDINATE = CR
             SY%SYMBOL = ATELE
