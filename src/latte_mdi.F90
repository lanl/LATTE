...
MODULE LATTE_MDI
  ...
  USE MDI, ONLY : MDI_INIT, MDI_SEND, MDI_INT, MDI_CHAR, MDI_NAME_LENGTH, &
       ...
       MDI_PLUGIN_GET_ARG, MDI_PLUGIN_GET_ARGC, MDI_SET_PLUGIN_STATE
...
CONTAINS
  ...
  FUNCTION MDI_PLUGIN_INIT_LATTE_MDI(PLUGIN_STATE) BIND (C, NAME="MDI_Plugin_init_latte_mdi")
    ...
    CHARACTER(LEN=1024) :: OTHER_OPTIONS

    CALL MDI_SET_PLUGIN_STATE(PLUGIN_STATE, IERR)

    ! start LATTE running as an MDI engine
    CALL MDI_ENGINE_INVOKE(OTHER_OPTIONS)
    
    write(*,*)"DEBG: After MDI_ENGINE_INVOKE, at MDI_PLUGIN_INIT_LATTE_MDI"
    MDI_PLUGIN_INIT_LATTE_MDI = 0
    write(*,*)"DEBG: After MDI_ENGINE_INVOKE, at MDI_PLUGIN_INIT_LATTE_MDI, END of the func call"
  ...
  SUBROUTINE MDI_ENGINE_INVOKE(OTHER_OPTIONS)

    USE COULOMBARRAY, ONLY : K1_LIST, K2_LIST, K3_LIST, KSQ_LIST
    USE SETUPARRAY, ONLY : NORBINDEX
    USE TIMER_MOD, ONLY : TSTART, TTOTAL, TCOUNT, TNAME, TAVG, TSUM, TPERCENT
    ...
    RESPONSE_LOOP: DO
      ...
      IF (EXITFLAG) THEN 
        WRITE(*,*)"DEBUG: EXITFLAG was set to True, now exiting the loop, at MDI_ENGINE_INVOKE"
        EXIT
      ENDIF
    ...
    IF(ALLOCATED(K1_LIST)) DEALLOCATE(K1_LIST)
    IF(ALLOCATED(K2_LIST)) DEALLOCATE(K2_LIST)
    IF(ALLOCATED(K3_LIST)) DEALLOCATE(K3_LIST)
    IF(ALLOCATED(KSQ_LIST)) DEALLOCATE(KSQ_LIST)
    IF(ALLOCATED(NORBINDEX)) DEALLOCATE(NORBINDEX)

    IF(ALLOCATED(TSTART)) DEALLOCATE(TSTART)
    IF(ALLOCATED(TTOTAL)) DEALLOCATE(TTOTAL)
    IF(ALLOCATED(TCOUNT)) DEALLOCATE(TCOUNT)
    IF(ALLOCATED(TNAME)) DEALLOCATE(TNAME)
    IF(ALLOCATED(TAVG)) DEALLOCATE(TAVG)
    IF(ALLOCATED(TSUM)) DEALLOCATE(TSUM)
    IF(ALLOCATED(TPERCENT)) DEALLOCATE(TPERCENT)
  ...
  SUBROUTINE EXECUTE_COMMAND(COMMAND, MDICOMM, IERR)
    ...
    CASE("EXIT")
      WRITE(*,*)"DEBUG: EXIT flag is received at EXECUTE_COMMAND on latte_mdi"
      EXITFLAG = .TRUE.
...
PROGRAM LATTE_MDI_ENGINE !Program
  ...
  WRITE(*,*)"DEBUG: Outside the MDI_ENGINE_INVOKE, about to do MPI_FINALIZE at LATTE_MDI_ENGINE"

  !CALL MPI_FINALIZE(IERR)

  WRITE(*,*)"DEBUG: At the end of LATTE_MDI_ENGINE program." 
