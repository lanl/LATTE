  ...
  SUBROUTINE EXECUTE_COMMAND(COMMAND, MDICOMM, IERR)
    ...
    CASE ( "<PE" )
      ...
      IF ( STATE == ">" ) THEN
        ...
        WRITE(*,*)"BOX",BOX
        CALL LATTE(NTYPES, TYPES, COORDS, BOX, FORCES, &
        MAXITER, VENERGTMP, STRESS, NEWSYSTEM, EXISTERROR, SYMB, FNAME)
        WRITE(*,*)"FORCESBACK",FORCES(1:3,1),FORCES(1:3,2)
        NEWSYSTEM = 0
        VENERG = VENERGTMP*eV2H
    ...
    CASE ( "<STRESS" )
      ...
      IF ( STATE == ">" ) THEN
        ...
        CALL LATTE(NTYPES, TYPES, COORDS, BOX, FORCES, &
        MAXITER, VENERG, STRESSTMP, NEWSYSTEM, EXISTERROR, SYMB, FNAME)
        DEALLOCATE(STRESSTMP)  
        NEWSYSTEM = 0
        STRESS = STRESSTMP*eV2H/(Ang2Bohr**3)
