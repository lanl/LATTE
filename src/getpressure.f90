...
SUBROUTINE GETPRESSURE
  ...
  IF (PPOTON .NE. 0) VIRIAL = VIRIAL + VIRPAIR

  IF (ELECTRO .EQ. 1) VIRIAL = VIRIAL + VIRCOUL

  IF (BASISTYPE .EQ. "NONORTHO") VIRIAL = VIRIAL + VIRPUL

  IF (PLUSDON .EQ. 1) VIRIAL = VIRIAL + VIRPLUSD
  ...
  IF (ELECTRO .EQ. 1 .AND. ELECMETH .EQ. 0) THEN

     QVIRIAL = (KECONST*PI/(TWO*SYSVOL*CALPHA*CALPHA))*REAL(CHARGE*CHARGE)

     VIRIAL(1) = VIRIAL(1) - QVIRIAL
     VIRIAL(2) = VIRIAL(2) - QVIRIAL
     VIRIAL(3) = VIRIAL(3) - QVIRIAL
