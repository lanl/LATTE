...
SUBROUTINE TBFORCESPROGRESS
  ...
  TYPE(BML_MATRIX_T) :: AUX_BML, AUX1_BML, AUX2_BML, HAM_BML, RHO_BML
  ...
  IF (SPINON .EQ. 0) THEN
     ...
     CALL BML_ZERO_MATRIX(BML_MATRIX_DENSE, &
          & BML_ELEMENT_REAL,LATTEPREC,HDIM,HDIM,RHO_BML)
     CALL BML_IMPORT_FROM_DENSE(LT%BML_TYPE, &
          & H, HAM_BML, LT%THRESHOLD, HDIM)
     CALL BML_IMPORT_FROM_DENSE(LT%BML_TYPE, &
          & BO, RHO_BML, LT%THRESHOLD, HDIM)
     ...
     IF (KBT .GT. 0.0000001) THEN
        ...
        CALL BML_MULTIPLY(ZMAT_BML,ZMAT_BML,AUX_BML,1.0_dp,0.0_dp,NUMTHRESH)

        ! S^-1 * H

        CALL BML_MULTIPLY(AUX_BML,HAM_BML,AUX1_BML,1.0_dp,0.0_dp,NUMTHRESH)

        ! (S^-1 * H)*RHO

        CALL BML_MULTIPLY(AUX1_BML,RHO_BML,AUX_BML,1.0_dp,0.0_dp,NUMTHRESH)
     ...
     ELSE
        ...
        CALL BML_MULTIPLY(RHO_BML,HAM_BML,AUX1_BML,1.0_dp,0.0_dp,NUMTHRESH)
  ...
  ELSE
     ...
     IF (KBT .GT. 0.0000001) THEN
        ...
        CALL BML_MULTIPLY(HAMUP_BML,RHOUP_BML,AUX1_BML,1.0_dp,0.0_dp,NUMTHRESH)

        ! (Hup * rhoup) + Hdown*rhodown 

        CALL BML_MULTIPLY(HAMDN_BML,RHODN_BML,AUX1_BML,1.0_dp,1.0_dp,NUMTHRESH)

        CALL BML_MULTIPLY(AUX_BML,AUX1_BML,AUX2_BML,1.0_dp,0.0_dp,NUMTHRESH)


     ELSE


        CALL BML_MULTIPLY(RHOUP_BML,HAMUP_BML,AUX_BML,1.0_dp,0.0_dp,NUMTHRESH)

        CALL BML_MULTIPLY(AUX_BML,RHOUP_BML,AUX2_BML,1.0_dp,0.0_dp,NUMTHRESH)
        
        CALL BML_MULTIPLY(RHODN_BML,HAMDN_BML,AUX_BML,1.0_dp,1.0_dp,NUMTHRESH)
        
        CALL BML_MULTIPLY(AUX_BML,RHODN_BML,AUX2_BML,1.0_dp,1.0_dp,NUMTHRESH)
