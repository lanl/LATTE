...
FUNCTION WIGNERD(L, M, MP, COSBETA)
  ...
  PREF =  MINUSONEPOWERS(L - MP) * &
       SQRT( REAL(FACTORIALLIST(L + M) * FACTORIALLIST(L - M) * &
       FACTORIALLIST(L + MP) * FACTORIALLIST(L - MP)) ) / REAL(2**L)
  ...
  DO K = MAX(0, - M - MP), MIN(L - M, L - MP)
     ...
     NUMER = MINUSONEPOWERS(K) * &
          ONEMCOSBETA ** (L - K - HALFMPMP) * &
          ONEPCOSBETA ** (K + HALFMPMP)

     DENOM = REAL(FACTORIALLIST(K) * FACTORIALLIST(L - M - K) * &
          FACTORIALLIST(L - MP - K) * FACTORIALLIST(M + MP + K))
