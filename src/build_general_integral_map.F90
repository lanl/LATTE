...
SUBROUTINE BUILD_GENERAL_INTEGRAL_MAP
  ...
  IF (EXISTERROR) RETURN; IF (VERBOSE >= 2) WRITE(*,*)"In build_integral_map.F90 ..."
  IF (VERBOSE >= 3)MLSI = TIME_MLS()
  ...
  !$OMP SHARED(BTYPE,BTYPE_INT,NOINT,L_MAX,MP_MAX) &
  !$OMP PRIVATE(I) 
  DO I = 1, NOINT
  ...
  ALLOCATE(IGL_MAP(0:MP_MAX, 0:L_MAX, 0:L_MAX, NUM_ATOMTYPES, NUM_ATOMTYPES))

  IGL_MAP = 0

  !$OMP PARALLEL DO DEFAULT (NONE) &
  !$OMP SHARED(L_MAX) &
  !$OMP SHARED(NOINT,ELE1,ELE2,BTYPE) &
  !$OMP SHARED(IGL_MAP) &
  !$OMP PRIVATE(I,J,L1,L2,IP1,IP2)& 
  !$OMP PRIVATE(MP,IGLTYPE,IC,BREAKLOOP) 
  DO I = 1, NUM_ATOMTYPES 
     DO J = 1, NUM_ATOMTYPES 
        ...
        DO L1 = 0, L_MAX
           DO L2 = 0, L_MAX
              ...
              DO MP = 0, MIN(L1, L2)
                 ...
                 ! Easier case first ATOMTYPES(I) = ATOMTYPES(J)

                 IF (ATOMTYPES(I) .EQ. ATOMTYPES(J)) THEN
                    

                    DO IC = 1, NOINT
!                       write(*,*) "ELE1", ELE1
!                       write(*,*) "ELE2", ELE2
                       IF (ATOMTYPES(I) .EQ. ELE1(IC) .AND. ATOMTYPES(J) .EQ. ELE2(IC) .AND. &
                 ...
                 ELSE 
                    ...
                    IF (L1 .EQ. L2) THEN ! This is a special case sss, pps, ppp etc.
                       ...
                       DO IC = 1, NOINT
                          ...
                          IF (((ATOMTYPES(I) .EQ. ELE1(IC) .AND. ATOMTYPES(J) .EQ. ELE2(IC)) .OR. &
                               (ATOMTYPES(I) .EQ. ELE2(IC) .AND. ATOMTYPES(J) .EQ. ELE1(IC))) .AND. &
                               IGLTYPE .EQ. BTYPE(IC)) THEN
                    ...
                    ELSE  ! L1 .NE. L2
                       ...
                       ELSE
                          ...
                          DO IC = 1, NOINT
                             ...
                             IF ((ATOMTYPES(I) .EQ. ELE2(IC) .AND. ATOMTYPES(J) .EQ. ELE1(IC)) .AND. &
                                  IGLTYPE .EQ. BTYPE(IC)) THEN
  ...
  ENDDO
  !$OMP END PARALLEL DO
  IF (VERBOSE>=3) WRITE(*,*)"Time for BUILD_INTEGRAL_MAP", TIME_MLS() - MLSI

  RETURN
  
END SUBROUTINE BUILD_GENERAL_INTEGRAL_MAP
