...
SUBROUTINE HUBBARDFORCE  ! AND ENERGY
  ...
  D = 0.5D0*BO
  P = 0.5D0*BO
  call DGEMM('N','N',HDIM,HDIM,HDIM,ONE,BO,HDIM,SMAT,HDIM,ZERO,PS,HDIM)
  ...
  write(*,*) 'HubbardForce UD =', UD(1:2,1:3)
  write(*,*) 'HubbardForce UP =', UP(1:2,1:3)
...
!call MMult(ONE,UP,PS,ZERO,UPSP,'N','T',HDIM)
  ...
  write(*,*) 'HubbardForce UPSP =', UPSP(1:2,1:3)
  write(*,*) 'HubbardForce UDSP =', UDSP(1:2,1:3)
  write(*,*) 'HubbardForce UPSD =', UDSP(1:2,1:3)
...
!  call MMult(ONE,A,SMAT,ZERO,B,'N','N',HDIM)
  ...
  enddo
  enddo
  write(*,*) 'HubbardForce A =', A(1:2,1:3)
  AHub = A
  CALL HUBFCONTR_SP    ! Negative sign already included
  HubForce = 0.5D0*HubForce   ! No negative sign
...
!!$OMP END PARALLEL DO
  ...
  write(*,*) ' HUBBARDFORCE = ', HubForce(1:3,:)
