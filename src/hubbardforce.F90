...
SUBROUTINE HUBBARDFORCE  ! AND ENERGY
  ...
  REAL(LATTEPREC), ALLOCATABLE :: UD(:,:), UP(:,:), DS(:,:)
  REAL(LATTEPREC), ALLOCATABLE :: UPSD(:,:), PSUD(:,:)
  REAL(LATTEPREC), ALLOCATABLE :: A(:,:), B(:,:)
  ...
  ALLOCATE(UD(HDIM,HDIM), UP(HDIM,HDIM), DS(HDIM,HDIM))
  ALLOCATE(UPSD(HDIM,HDIM), PSUD(HDIM,HDIM),A(HDIM,HDIM), B(HDIM,HDIM))
  ...
  IF (KON==0) then
    ...
    do I = 1, HDIM
    do J = 1, HDIM
       UD(I,J) = DFTB_U(I)*BO(I,J) * 0.5_DP
       UP(I,J) = DFTB_U(I)*PNO(I,J) * 0.5_DP
    enddo
    enddo
  
    call DGEMM('N','N',HDIM,HDIM,HDIM,0.5_DP,PNO,HDIM,SMAT,HDIM,ZERO,PS,HDIM)
    call DGEMM('N','N',HDIM,HDIM,HDIM,0.5_DP,BO,HDIM,SMAT,HDIM,ZERO,DS,HDIM)
  ...
  DEALLOCATE(UD, UP, DS, UPSD, PSUD, A, B)
