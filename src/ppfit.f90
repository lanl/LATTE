...
SUBROUTINE PPFIT
  ...
  DFTCE = -37.858854555371*27.211385
  DFTHE = -0.502159745168*27.211385
  ...
  ALLOCATE(BONDFORCES(3,TOTAT), FPAIR(3,TOTAT), DFTENERGY(NSNAP))
  ALLOCATE(NC(NSNAP), NH(NSNAP))
  ...
  DO I = 1, NSNAP
     ...
     DO J = 1, ATINMOL(I)
        ...
        READ(60,*) SPEC(COUNT), COORDS(1,COUNT), COORDS(2,COUNT), &
             COORDS(3,COUNT), FORCES(1,COUNT), FORCES(2,COUNT), &
             FORCES(3,COUNT)
...
10 FORMAT(A1,6F12.6)
  ...
  DFTENERGY = DFTENERGY*27.211385
  FORCES = -FORCES*27.211385/0.591772

  ! Turn the DFT energies into atomization energies
  ...
  DO II = 1, NSNAP
     ...
     TOTE = TRRHOH - ECOUL - ENTE 

     ! Take off the contributions to the total energy from the bond term
  ...
  DO I = 1, PP2FIT
     WRITE(6,20) PPELE1(I), PPELE2(I), PPBEST(1,I), PPBEST(2,I), &
          PPBEST(3,I), PPBEST(4,I), PPBEST(5,I), POTCOEF(6,I), POTCOEF(7,I), &
