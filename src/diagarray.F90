...
MODULE DIAGARRAY
  ...
  REAL(LATTEPREC), ALLOCATABLE :: DVALS(:) 
  REAL(LATTEPREC), ALLOCATABLE :: FERMIOCC(:)  !! ANDERS_CHANGE
  ...
  REAL(LATTEPREC), ALLOCATABLE :: KEVALSUP(:,:), KEVALSDOWN(:,:)
  COMPLEX(LATTEPREC), ALLOCATABLE :: KEVECS(:,:,:)
  COMPLEX(LATTEPREC), ALLOCATABLE :: KEVECSUP(:,:,:), KEVECSDOWN(:,:,:)
  REAL(LATTEPREC), PARAMETER :: EXPTOL = 30.0
  REAL(LATTEPREC) :: NUMLIMIT
#ifdef PROGRESSON
  TYPE(BML_MATRIX_T) :: EVECS_BML
#endif
  !  NUMLIMIT = EXP(-EXPTOL)
