...
SUBROUTINE PULAY_SPPROGRESS
  ...
  TYPE(BML_MATRIX_T) :: H_BML, BO_BML, X2HRHO_BML
  TYPE(BML_MATRIX_T) :: HUP_BML, HDOWN_BML, RHOUP_BML, RHODOWN_BML, X2HRHOUP_BML
  ...
  IF (SPINON .EQ. 0) THEN
     ...
     CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, LT%MDIM, H_BML)
     CALL BML_IMPORT_FROM_DENSE(LT%BML_TYPE, &
          H, H_BML, ZERO, LT%MDIM)

     !! Convert the density matrix to bml format
     CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, LT%MDIM, BO_BML)
     CALL BML_IMPORT_FROM_DENSE(LT%BML_TYPE, &
          BO, BO_BML, ZERO, LT%MDIM)

     !! Allocate a bml matrix for the Pulay component matrix
     CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, LT%MDIM, X2HRHO_BML)

     IF (KBT .GT. 0.0000001) THEN

        CALL PRG_PULAYCOMPONENTT(BO_BML,H_BML,ZMAT_BML,X2HRHO_BML,LT%THRESHOLD &
             &,LT%MDIM,LT%BML_TYPE,VERBOSE)
     ...
     ELSE
        ...
        CALL PRG_PULAYCOMPONENT0(BO_BML,H_BML,X2HRHO_BML,LT%THRESHOLD &
             &,LT%MDIM,LT%BML_TYPE,VERBOSE)
     ...
     CALL BML_EXPORT_TO_DENSE(X2HRHO_BML, X2HRHO)
     CALL BML_DEALLOCATE(X2HRHO_BML)

  ELSE

     !! Allocate a bml matrix for half of the Pulay component matrix
     CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, LT%MDIM, X2HRHOUP_BML)

     !! Convert Hamiltonian (Up) to bml format
     CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, LT%MDIM, HUP_BML)
     CALL BML_IMPORT_FROM_DENSE(LT%BML_TYPE, &
          HUP, HUP_BML, ZERO, LT%MDIM)
     !! Convert the density matrix (RhoUp) to bml format
     CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, LT%MDIM, RHOUP_BML)
     CALL BML_IMPORT_FROM_DENSE(LT%BML_TYPE, &
          RHOUP, RHOUP_BML, ZERO, LT%MDIM)

     IF (KBT .GT. 0.0000001) THEN
        CALL PRG_PULAYCOMPONENTT(RHOUP_BML,HUP_BML,ZMAT_BML,X2HRHOUP_BML,LT%THRESHOLD &
             &,LT%MDIM,LT%BML_TYPE,VERBOSE)
     ELSE
        CALL PRG_PULAYCOMPONENT0(RHOUP_BML,HUP_BML,X2HRHOUP_BML,LT%THRESHOLD &
             &,LT%MDIM,LT%BML_TYPE,VERBOSE)
     ENDIF

     CALL BML_DEALLOCATE(RHOUP_BML)
     CALL BML_DEALLOCATE(HUP_BML)

     !! Convert Hamiltonian (Down) to bml format
     CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, LT%MDIM, HDOWN_BML)
     CALL BML_IMPORT_FROM_DENSE(LT%BML_TYPE, &
          HDOWN, HDOWN_BML, ZERO, LT%MDIM)
     !! Convert the density matrix (RhoDown) to bml format
     CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, LT%MDIM, RHODOWN_BML)
     CALL BML_IMPORT_FROM_DENSE(LT%BML_TYPE, &
          RHODOWN, RHODOWN_BML, ZERO, LT%MDIM)

     !! Allocate a bml matrix for the other half pulay component matrix
     CALL BML_ZERO_MATRIX(LT%BML_TYPE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, LT%MDIM, X2HRHO_BML)

     IF (KBT .GT. 0.0000001) THEN
        CALL PRG_PULAYCOMPONENTT(RHODOWN_BML,HDOWN_BML,ZMAT_BML,X2HRHO_BML,LT%THRESHOLD &
             &,LT%MDIM,LT%BML_TYPE,VERBOSE)
     ELSE
        CALL PRG_PULAYCOMPONENT0(RHODOWN_BML,HDOWN_BML,X2HRHO_BML,LT%THRESHOLD &
             &,LT%MDIM,LT%BML_TYPE,VERBOSE)
     ENDIF

     CALL BML_DEALLOCATE(RHODOWN_BML)
     CALL BML_DEALLOCATE(HDOWN_BML)

     CALL BML_ADD_DEPRECATED(2.0d0,X2HRHO_BML,2.0d0,X2HRHOUP_BML)

     CALL BML_DEALLOCATE(X2HRHOUP_BML)
     CALL BML_EXPORT_TO_DENSE(X2HRHO_BML, X2HRHO)
     CALL BML_DEALLOCATE(X2HRHO_BML)
...
END SUBROUTINE PULAY_SPPROGRESS
