...
!       ORTHOH, ORTHOH_BML, ZERO, HDIM)
  ...
  IF (KBT .GT. 0.000001) THEN  ! This bit is for a finite electronic temperature
    ...
    ELSE
      ...
      CALL BML_ZERO_MATRIX(BML_MATRIX_DENSE, BML_ELEMENT_REAL, &
          LATTEPREC, HDIM, HDIM, EVECS_BML)
      ! Only perform diagonalization if we haven't done it (eigenvalues = 0)
      IF (MAXVAL(EVALS) == 0.0D0 .AND. MINVAL(EVALS) == 0.0D0)THEN
        call nvtxStartRange("bml_diagonalize",4)
      ...
      CALL BML_IMPORT_FROM_DENSE(BML_MATRIX_DENSE, &
          EVECS, EVECS_BML, ZERO, HDIM)
      call nvtxStartRange("Build_DM",8)  
      CALL PRG_BUILD_DENSITY_FROMEVALSANDEVECTS(EVECS_BML, EVALS, ORTHOBO_BML, NUMTHRESH,&
          & BNDFIL, KBT, CHEMPOT, VERBOSE)
      call nvtxEndRange
  ...
  ELSE ! This bit is for zero electronic temperature
    ...
    ELSE
      ...
      CALL PRG_BUILD_DENSITYGP_T0(ORTHOH_BML, G_BML, ORTHOBO_BML, NUMTHRESH, BNDFIL, E0, GRAPH_NPARTS, VERBOSE)
