...
10   FORMAT(F12.6, 1X, 500(G14.6, 1X))
  ...
  ELSEIF (SCLTYPE .EQ. "GAUSSIAN") THEN
     ...
     OPEN(UNIT=40, STATUS="UNKNOWN", FILE="S_scaling_GAUSSIAN.dat")
     ...
     DO I = 1, NOSAMPLES
        ...
        DO J = 1, NOINT
           ...
           IF (MAGR .LE. OVERL(9,J)) THEN
              
              TMP = (OVERL(1,J)*MAGR**3)*EXP(-OVERL(2,J)*MAGR**2) + &
                   (OVERL(3,J)*MAGR**2)*EXP(-OVERL(4,J)*MAGR**2) + &
                   (OVERL(5,J)*MAGR)*EXP(-OVERL(6,J)*MAGR**2) + &
                   (OVERL(7,J))*EXP(-OVERL(8,J)*MAGR**2) 

           ELSEIF (MAGR .GT. OVERL(9,J) .AND. MAGR .LE. OVERL(10,J)) THEN
