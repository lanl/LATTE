...
MODULE LATTEPARSER_LATTE_MOD
  ...
  TYPE, PUBLIC :: LATTE_TYPE
     ...
     !> Total number of steps for MD simulation.
     CHARACTER(1000) :: PARAMPATH

     !> File containing coordinates.
     CHARACTER(1000) :: COORDSFILE
...
CONTAINS
  ...
  SUBROUTINE PARSE_CONTROL(FILENAME)
    ...
    IMPLICIT NONE
    INTEGER, PARAMETER :: NKEY_CHAR = 8, NKEY_INT = 54, NKEY_RE = 21, NKEY_LOG = 2
    CHARACTER(LEN=*) :: FILENAME

    !Library of keywords with the respective defaults.
    CHARACTER(LEN=50), PARAMETER :: KEYVECTOR_CHAR(NKEY_CHAR) = [CHARACTER(LEN=1000) :: &
         'JOBNAME=','BASISTYPE=','SP2CONV=','RELAXTYPE=','PARAMPATH=','COORDSFILE=', &
         'SCLTYPE=','VIBSAVE=' ]
    CHARACTER(LEN=1000) :: VALVECTOR_CHAR(NKEY_CHAR) = [CHARACTER(LEN=1000) :: &
    ...
    VIBANALYSIS = VALVECTOR_INT(54)
    VIBSAVE = VALVECTOR_CHAR(8)
