...
SUBROUTINE DFDX_HS(I, J, L1, L2, M1, M2, R, ALPHA, COSBETA, DFDA, DFDB, DFDR, &
     SDFDA, SDFDB, SDFDR)
  ...
  REAL(LATTEPREC) :: ALPHA, COSBETA, R
  REAL(LATTEPREC) :: DFDA, DFDB, DFDR, SDFDA, SDFDB, SDFDR 
  REAL(LATTEPREC), EXTERNAL :: WIGNERD, AM, BM
  REAL(LATTEPREC) :: WIG1, WIG2, SL1, SL2, AM1, AM2, BM1, BM2, TL1, TL2
  ...
  REAL(LATTEPREC) :: BONDINT, DBONDINT, OVERLAPINT, DOVERLAPINT, TMP
  ...
  CALL DUNIVSCALE(I, J, L1, L2, 0, R, "H", BONDINT, DBONDINT)
  ...
  IF (BASISTYPE .EQ. "NONORTHO") THEN
     ...
     CALL DUNIVSCALE(I, J, L1, L2, 0, R, "S", OVERLAPINT, DOVERLAPINT)
  ...
  DFDA = TMP*BONDINT
  SDFDA = TMP*OVERLAPINT
  
  TMP = TWO * AM1 * AM2  * &
       (DWIG1 * WIG2 + WIG1 * DWIG2)

  DFDB = TMP * BONDINT
  SDFDB = TMP * OVERLAPINT

  TMP = TWO * AM1 * AM2 * WIG1 * WIG2 

  DFDR = TMP * DBONDINT
  SDFDR = TMP * DOVERLAPINT

  DO MP = 1, MIN(L1, L2)
     
     CALL DUNIVSCALE(I, J, L1, L2, MP, R, "H", BONDINT, DBONDINT)
     ...
     IF (BASISTYPE .EQ. "NONORTHO") THEN
        ...
        CALL DUNIVSCALE(I, J, L1, L2, MP, R, "S", OVERLAPINT, DOVERLAPINT)
     ...
     TMP = (DSLA1*SL2 + SL1*DSLA2 + DTLA1*TL2 + TL1*DTLA2)

     DFDA = DFDA + TMP*BONDINT
     SDFDA = SDFDA + TMP*OVERLAPINT

     TMP = (DSLB1*SL2 + SL1*DSLB2 + DTLB1*TL2 + TL1*DTLB2)

     DFDB = DFDB + TMP*BONDINT
     SDFDB = SDFDB + TMP*OVERLAPINT

     TMP = (SL1 * SL2 + TL1 * TL2)

     DFDR = DFDR + TMP*DBONDINT
     SDFDR = SDFDR + TMP*DOVERLAPINT

  ENDDO

  RETURN 

END SUBROUTINE DFDX_HS
